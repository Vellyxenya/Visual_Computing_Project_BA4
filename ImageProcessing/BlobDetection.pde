...
class BlobDetection {
  ...
  private static final int NB_PRESET_COLORS = 900;
  ...
  PImage findConnectedComponents(PImage input, boolean onlyBiggest, color fill) {
      ...
      for (int x = 0; x < input.width; x++) {
        if (input.pixels[y*input.width+x] != color(0)) {
          ...
          for (int j = y-N/2; j <= y; j++) {
            for (int i = x-N/2; i <= x+N/2 && n > 0; i++, n--) {
    ...
    //Complete labelsEquivalences
    int k = 1;
    for(TreeSet<Integer> s : labelsEquivalences) {
      for(int i : s) {
        labelsEquivalences.get(i-1).add(k);
      }
      k++;
    }
